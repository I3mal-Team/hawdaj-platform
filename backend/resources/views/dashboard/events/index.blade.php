...
@push('js')
    <script>
        ...
        $("#delete_selected").on("click", function(e) {
            ...
            $.ajax({
                url: "{{ route('dashboard.events.destroy_selected') }}",
                type: 'delete',
                data: {
                    'array_ids': listArray,
                },
    ...
    <script>
        $('.activate').change(function() {
            ...
            $.ajax({
                url: "{{ Route('dashboard.events.active') }}",
                method: 'POST',
                data: {
                    'checked': checked,
                    'eventId': place_id,
            ...
            <div class="card card-custom">
                <div class="card-header flex-wrap border-0 pt-6 pb-0">
                    <div class="card-title">
                        <h3 class="card-label">{{ __('dashboard.events_list') }}</h3>
                    </div>
                    <div class="card-toolbar">

                        <a href="#" class="btn btn-danger ml-2 mr-2 delete_selected_modal "
                            style="color: white !important; display:none" data-toggle="modal"
                            data-target="#delete_selected_modal">
                            <i class="flaticon2-trash trash-icon" style="color: white !important;"></i>@lang('dashboard.delete_selected')</a>

                        @if (!request('archive') && \App\Models\Event::onlyTrashed()->first())
                            <a href="{{ route('dashboard.events.index', ['archive' => 1]) }}" class="btn btn-warning m-2">
                                <i class="la la-trash"></i>@lang('dashboard.archive')</a>
                        @endif

                        @if (!empty($events->first()) && !request('archive'))
                            <a href="{{ route('dashboard.events.create') }}" class="btn btn-primary"><i
                                    class="la la-plus"></i>@lang('dashboard.new_event')</a>
                        @else
                            <a href="{{ route('dashboard.events.index') }}"
                                class="btn btn-primary m-2">@lang('dashboard.events')</a>
                        @endif
                ...
                <div class="card-header flex-wrap border-0 pt-6 pb-0">
                    ...
                    <form action="{{ route('dashboard.events.index') }}" method="get"
                        style="width: -webkit-fill-available;">

                        <div class="row">

                            <div class="col-md-3 mb-3">
                                <input name="title" class="form-control" placeholder="@lang('dashboard.search')"
                                    value="{{ request()->title }}">
                            </div>
                            ...
                            <div class="col-md-3">
                                ...
                                @if(request()->user_places || request()->title)
                                    <a href="{{ route('dashboard.events.index') }}" class="btn btn-secondary">@lang('dashboard.reset')</a>
                                @endif
                            </div>
                ...
                @if (!empty($events->first()))
                            ...
                            <tbody>
                                @if (!empty($events->first()))
                                    @foreach ($events as $index => $event)
                                        <tr id="row-{{ $event->id }}" @if($event->user_id) style="background-color: #fff3cd;" @endif>
                                            <td><input type="checkbox" name="ids" value="{{ $event->id }}"
                                                    class="checkbox checkboxD check-delete "></td>
                                                    <td>{{($index + 1) + ((request('page' , 1)-1) * request('per_page' , 10)) }}</td>
                                            <td>
                                                @php
                                                    $descriptionImage = extractImageFromHtml($event->description ?? '');
                                                @endphp
                                                @if($descriptionImage)
                                                ...
                                                @elseif($event->image)
                                                <a style="width: 200px;" href="{{ resolvePhoto($event->image) }}" target="_blank">
                                                    <div class="d-flex align-items-center">
                                                        <div class="symbol symbol-60 flex-shrink-0">
                                                            <div class="symbol-label" style="background-image: url({{ resolvePhoto($event->image) }})">
                                                            </div>
                                                        </div>
                                                    </div>
                                                </a>
                                                ...
                                                @if (isset($event->categories))
                                                    @if (count($event->categories) > 0)
                                                        @foreach ($event->categories as $category)
                                                            <span
                                                                class="badge badge-pill badge-primary">{{ \App\Models\CategoryOfEvent::where('id', (int) $category)->first()->name ?? '---' }}</span>
                                                        @endforeach
                                                    @endif
                                                @endif
                                            </td> --}}
                                            <td>
                                                @if($event->user_id && $event->user)
                                                    <span class="badge badge-warning">
                                                        <i class="flaticon2-user"></i> {{ $event->user->first_name . ' ' . $event->user->last_name }}
                                                    </span>
                                                @else
                                                    <span class="text-muted">---</span>
                                                @endif
                                            ...
                                            @if(auth()->user()->can('patch-event'))
                                                <td class="text-center">
                                                    <span class="switch switch-outline switch-icon switch-success">
                                                        <label>
                                                            <input id="" class="activate"
                                                                data-item-id="{{ $event->id }}" type="checkbox"
                                                                @if ($event->active) checked @endif name="active"
                                                                value="" />
                                                            <span></span>
                                                        </label>
                                                    </span>
                                            ...
                                            <td>{{ dateFormat($event->updated_at) ?? '---' }}</td>
                                            <td>

                                                @if (request('archive', 0))
                                                    <a class="btn btn-sm btn-clean btn-icon btn-icon-md restore-button"
                                                        title="{{ __('dashboard.restore') }}" data-toggle="modal"
                                                        data-target="#restore_modal"
                                                        data-url="{{ route('dashboard.events.restore', $event->id) }}"
                                                        data-item-id="{{ $event->id }}">
                                                        <i class="flaticon2-refresh refresh-icon"></i>
                                                    </a>
                                                    <a class="btn btn-sm btn-clean btn-icon btn-icon-md delete-button"
                                                        title="{{ __('dashboard.force_delete') }}" data-toggle="modal"
                                                        data-target="#delete_modal"
                                                        data-url="{{ route('dashboard.events.destroy', [$event->id, 'archive' => request('archive')]) }}"
                                                        data-item-id="{{ $event->id }}">
                        ...
                        {!! str_replace('/?', '?', $events->withQueryString()->render()) !!}
                ...
                @else
                    <div class="card-body">
                        ...
                        <div class="card-toolbar"
                            style=" text-align: center; background-color: #f2f3f8 ;padding: 100px;  color: #101010">
                            <div style="font-size: large">
                                <i class="la la-briefcase mb-2 " style="font-size: xxx-large; "></i><br>
                                @lang('dashboard.no_data')
                            </div>

                            <a href="{{ route('dashboard.events.create') }}" class="btn btn-primary mt-2">
                                <i class="la la-plus"></i>@lang('dashboard.new_event')</a>
                        </div>
                    </div>
                @endif
            </div>
