            ...
            <div class="card card-custom">
                <div class="card-header flex-wrap border-0 pt-6 pb-0">
                    <div class="card-title">
                        <h3 class="card-label">{{__('dashboard.cities_list')}}</h3>
                    </div>
                    <div class="card-toolbar">
                        <a href="{{route('dashboard.cities.create')}}"
                           class="btn btn-primary">
                            <i class="la la-plus"></i>@lang('dashboard.new_city')</a>
                    </div>
                </div>
                <div class="card-body">
                    ...
                    <table class="table table-bordered dataTable">
                        ...
                        <tr>
                            <th>#</th>
                            <th>@lang('dashboard.city_name')</th>
                        ...
                        @if(!empty($cities->first()))
                            @foreach($cities as $index => $city)
                                ...
                                <td>{{($index + 1) + ((request('page' , 1)-1) * request('per_page' , 10)) }}</td>
                                    <td>{{ $city->name?? '---'}}</td>
                                    <td>{{ $city->region_id?$city->region->name :'---'}}</td>
                                    <td>{{ dateFormat($city->created_at) ?? '---' }}</td>
                                    <td>{{ dateFormat($city->updated_at) ?? '---' }}</td>
                                    <td>
                    ...
                    {!! str_replace('/?', '?', $cities->withQueryString()->render()) !!}
