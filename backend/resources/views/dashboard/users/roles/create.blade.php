...
@section('page_header')
    ...
    <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
        ...
        <li class="breadcrumb-item text-muted">
            <a href="{{url('roles')}}" class="text-muted">@lang('dashboard.roles')</a>
        </li>
        <li class="breadcrumb-item text-muted">
                ...
                <div class="col-md-12">
                    <div class="card card-custom gutter-b ">
                        ...
                        <form id="form" novalidate="novalidate" class="kt-form kt-form--label-right" method="post"
                              action="{{ route('dashboard.roles.store') }}" enctype="multipart/form-data">
                            @csrf
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group validated">
                                            <label>@lang('dashboard.role_name')</label>
                                    ...
                                    <h2 class="ml-5 mt-8 mb-6">@lang('dashboard.permissions')</h2>
                                                    ...
                                                    <select   name="permissions[]" multiple="multiple"
                                                        ...
                                                        @foreach($permission as $operation)
                                                            <option value="{{ $operation->id }}"
                                                                {{ in_array($operation->id, old('permissions') ?? []) ? 'selected' : ''}}
                                                            >{{__('dashboard.'.explode(' ',ucfirst($operation->label))[0]) ?? '' }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            @endforeach
