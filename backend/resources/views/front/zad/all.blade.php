                ...
                <div class="container-fluid text-start">
                    ...
                    <form id="filterForm" class="filter-section__filters-form collapse">
                        <input type="text" class="form-control" value="{{ request('search') }}"
                               ...
                               name="search">
                        <select class="form-control select2-input" name="category_id" id="category_id"
                                onchange="get_sub_categories()">
                            <option selected value="0">{{ __('Category') }}</option>
                        ...
                        <select class="form-control select2-input" name="sub_category_id" id="sub_category_id">
                            <option selected value="0">{{ __('Sucategory') }}</option>
                        </select>
                        <select class="form-control select2-input" name="address_type" id="address_type">
                            <option selected value="0">{{ __('Location type') }}</option>
                            <option value="link" {{ request('address_type') == 'link' ? 'selected' : '' }}>link
                            </option>
                            <option value="map" {{ request('address_type') == 'map' ? 'selected' : '' }}>map
                            </option>
                        </select>
                ...
                <section class="filter-grid-section filter-grid-section--height">
                    <div class="container">
                        <div class="row">
                                        ...
                                        <div class="map-toggler">
                                            <button id="googleMapType" class="btn" onclick="initMap()">
                                                <svg xmlns="http://www.w3.org/2000/svg"
                                                     ...
                                                     height="1.3rem" viewBox="0 0 24 24" stroke-width="1.5"
                                            ...
                                            <button id="customMapType" class="btn" onclick="KSAMAP()">
                                                <svg xmlns="http://www.w3.org/2000/svg"
                                                     class="icon icon-tabler icon-tabler-map" width="1.3rem"
                                                     height="1.3rem"
                            ...
                            <div
                                class=" @if (request('address_type') == 'map') col-lg-4 col-xl-6 @else col-lg-12 col-xl-12 @endif ">
                                ...
                                <div class="filter-grid-section__container filter-grid-section--height">
                                    ...
                                    <div class="row mx-0">
                                        @foreach ($stores as $store)
                                            <div
                                                class="@if (request('address_type') == 'map') col-12 col-sm-6 col-md-4 col-lg-12 col-xl-6  @else col-3 @endif  mb-4 d-flex">
                                                   ...
                                                   class="palce-card card h-100">
                                                    ...
                                                    <div class="card-body pb-4">
                                                        ...
                                                        <div class="d-flex align-items-center">
                                                            ...
                                                            <span
                                                                class="views mr-3">({{ $store->review }} {{ __("Review") }})</span>
                                                        </div>
                                                    </div>
                                                </a>
                                            </div>
...
@section('scripts')
    ...
    <script>
        ...
        function displayStateOnGoogleMap(context) {
            ...
            if (lat && lng && x && y) {
                window.location.href = `/stores?lat=${lat}&lng=${lng}&x=${x}&y=${y}`;
            }
        }
        ...
        function initMap() {
            ...
            var data = '<?php echo json_encode($stores); ?>';
            var stores = JSON.parse(data);
            var allstores = stores.data;

            if (allstores.length > 0) {
                var location = {
                    lat: allstores[0].lat,
                    lng: allstores[0].long
                }
            } else {
                var location = {
                    lat: 24.7136,
        ...
        function get_sub_categories() {
            ...
            let sub_category_id = "{{ request('sub_category_id') }}"
            if (parent_id != '') {
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.length > 0) {
                            ...
                            response.forEach(element => {
                                result +=
                                    `
                                <option selected value="0"> {{__("Sub category")}}</option>
                                <option value="${element.id}" ${(sub_category_id == element.id) ? 'selected' : ''}>${element.name}</option> `
                            });
                            $("#sub_category_id").html('')
                            $("#sub_category_id").append(result)
                        } else {
                            let result = '<option selected value="0"> {{__("Sub category")}}</option>';
                            $("#sub_category_id").html('')
                            $("#sub_category_id").append(result)
                        }
                    },
        ...
        function addMarker(element, map, isActive = false) {
            ...
            function addMarker(element, map, isActive = false) {
                ...
                var marker = new google.maps.Marker({
                    ...
                    map: map,
                    icon: {
                        url: "{{ asset('') }}" + (isActive ? 'front_assets/imgs/marker-open.svg' :
