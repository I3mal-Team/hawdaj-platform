...
@section('page_header')
    <h5 class="text-dark font-weight-bold my-1 mr-5">@lang('dashboard.regions')</h5>
            ...
            <div class="card card-custom">
                <div class="card-header flex-wrap border-0 pt-6 pb-0">
                    <div class="card-title">
                        <h3 class="card-label">{{__('dashboard.regions_list')}}</h3>
                    </div>
                    <div class="card-toolbar">
                        <a href="{{route('dashboard.regions.create')}}"
                           class="btn btn-primary">
                            <i class="la la-plus"></i>@lang('dashboard.new_region')</a>
                    </div>
                </div>
                <div class="card-body">
                    ...
                    <table class="table table-bordered dataTable">
                        ...
                        <tr>
                            <th>#</th>
                            <th>@lang('dashboard.region_name')</th>
                        ...
                        <tbody>
                        @if(!empty($regions->first()))
                            @foreach($regions as $index => $region)
                                <tr id="row-{{$region->id}}">
                                <td>{{($index + 1) + ((request('page' , 1)-1) * request('per_page' , 10)) }}</td>
                                    <td>{{ $region->name?? '---'}}</td>
                                    <td>{{ dateFormat($region->created_at) ?? '---' }}</td>
                                    <td>{{ dateFormat($region->updated_at) ?? '---' }}</td>
                                    <td>
                                        <a href="{{route('dashboard.regions.edit', $region->id)}}"
                                        ...
                                        <a class="btn btn-sm btn-clean btn-icon btn-icon-md delete-button"
                                           ...
                                           data-url="{{ route('dashboard.regions.destroy',$region->id) }}"
                    ...
                    {!! str_replace('/?', '?', $regions->withQueryString()->render()) !!}
                </div>
            </div>
        </div>
    </div>
