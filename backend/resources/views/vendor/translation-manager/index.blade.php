        ...
        @if(isset($group))
            <form class="form-inline form-publish" method="POST"
                ...
                <button type="submit" class="btn btn-info"
                        data-disable-with="Publishing..">@lang('dashboard.publish_all')</button>
                <a href="<?= action('\Barryvdh\TranslationManager\Controller@getIndex') ?>"
                   class="btn btn-default">@lang('dashboard.back')</a>
            </form>
            @endif
            </p>
            <form role="form" method="POST"
                ...
                <div class="form-group">
                    <p>@lang('dashboard.choose_group') @endlang.</p>
                    <select name="group" id="group" class="form-control group-select">
                        @foreach ($groups as $key => $value)
                            @if(strlen($value) <=20)
                                <option
                                    value="{{$key}}" {{$key == $group ? ' selected' : ''}}>
                                    @lang('dashboard.' . \Illuminate\Support\Str::slug($value))
                                </option>
                            @endif
                        @endforeach;
                    </select>
            ...
            @if ($group)
                <form action="<?php echo action('\Barryvdh\TranslationManager\Controller@postAdd', array($group)) ?>"
                    ...
                    <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
                    <div class="form-group">
                        <label>@lang('dashboard.add_new_keys_to_this_group')</label>
                        <textarea class="form-control" rows="3" name="keys"
                                  placeholder="@lang('dashboard.add_per_line')"></textarea>
                    </div>
                    <div class="form-group">
                        <input type="submit" value="@lang('dashboard.add_keys')" class="btn btn-primary">
                    </div>
                </form>
                <form class="form-add-locale autotranslate-block-group hidden" method="POST" role="form"
                    ...
                    <div class="row">
                        <div class="col-sm-6">
                            <div class="form-group">
                                <label for="base-locale">@lang('dashboard.base_locale_for_auto_translations')</label>
                            ...
                            <div class="form-group">
                                <label for="new-locale">@lang('dashboard.enter_target_locale_key')</label>
                                <input type="text" name="new-locale" class="form-control" id="new-locale"
                                       placeholder="@lang('dashboard.enter_target_locale_key')"/>
                            </div>
                            <div class="form-group">
                ...
                <hr>
                <h4>@lang('dashboard.total'): <?= $numTranslations ?>, @lang('dashboard.changed')
                    : <?= $numChanged ?></h4>
                <table class="table">
                    <thead>
                    <tr>
                        <th width="15%">@lang('dashboard.key')</th>
                            <?php foreach ($locales as $locale): ?>
                        <th>@lang('dashboard.' . $locale)</th>
                        <?php endforeach; ?>
                    ...
                    <tr id="<?php echo htmlentities($key, ENT_QUOTES, 'UTF-8', false) ?>">
                        ...
                        <td>
                            <a href="#edit"
                               ...
                               data-title="@lang('dashboard.enter_translation')"><?php echo $t ? htmlentities($t->value, ENT_QUOTES, 'UTF-8', false) : '' ?></a>
                        </td>
                        <?php endforeach; ?>
                            <?php if ($deleteEnabled): ?>
                        <td>
            ...
            @else
                <fieldset>
                    <legend>@lang('dashboard.supported_locales')</legend>
                    <p>
                        @lang('dashboard.current_supported_locales'):
                    </p>
                    ...
                    <form class="form-add-locale" method="POST" role="form"
                        ...
                        <div class="form-group">
                            <p>
                                @lang('dashboard.enter_new_locale_key'):
                            </p>
                            <div class="row">
                                <div class="col-sm-3">
                ...
                <fieldset>
                    <legend>@lang('dashboard.export_all_translations')</legend>
                    <form class="form-inline form-publish-all" method="POST"
                          ...
                          data-remote="true" role="form"
                          data-confirm="@lang('dashboard.publish_all_translations_group').">
                        <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
