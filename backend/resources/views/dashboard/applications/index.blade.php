        ...
        <div class="card card-custom">
            <div class="card-header flex-wrap border-0 pt-6 pb-0">
                <div class="card-title">
                    <h3 class="card-label">{{__('dashboard.applications')}}</h3>
                </div>
                 <div class="card-toolbar">


                    @if(!empty($applications->first()) && !request('archive'))

                    <a href="{{route('dashboard.applications.create')}}" class="btn btn-primary"><i class="la la-plus"></i>@lang('dashboard.applications')</a>
                    @else

                    <a href="{{route('dashboard.applications.index')}}" class="btn btn-primary m-2">@lang('dashboard.applications')</a>
                    @endif
            ...
            <div class="card-header flex-wrap border-0 pt-6 pb-0">
                ...
                <form action="{{ route('dashboard.applications.index') }}" method="get" style="width: -webkit-fill-available;">
            ...
            <div class="card-body">
                ...
                <table class="table table-bordered dataTable2" id="">
                    ...
                    <tbody>
                        ...
                        <td>{{($index + 1) + ((request('page' , 1)-1) * request('per_page' , 10)) }}</td>
                            ...
                            <td>
                                @php
                                    $descriptionImage = extractImageFromHtml($application->description ?? '');
                                @endphp
                                @if($descriptionImage)
                                ...
                                <a style="width: 200px;" href="{{ resolvePhoto($application->image) }}" target="_blank">
                                    <div class="d-flex align-items-center">
                                        <div class="symbol symbol-60 flex-shrink-0">
                                            <div class="symbol-label" style="background-image: url({{ resolvePhoto($application->image) }})">
                                            </div>
                                        </div>
                                    </div>
                                </a>
                            ...
                            <td>{{ trans('dashboard.' . $application->type) }} </td>
                            ...
                            <td>
                                ...
                                <a class="btn btn-sm btn-clean btn-icon btn-icon-md delete-button" title="{{__('dashboard.force_delete')}}" data-toggle="modal" data-target="#delete_modal" data-url="{{ route('dashboard.applications.destroy',[$application->id , 'archive' => request('archive')]) }}" data-item-id="{{ $application->id }}">
                                    <i class="flaticon2-trash trash-icon"></i>
                                </a>
                                @else

                                <a href="{{route('dashboard.applications.edit', $application->id)}}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="{{__('dashboard.edit')}}">
                                    <i class="flaticon-edit-1 edit-icon"></i>
                                </a>
                                <a class="btn btn-sm btn-clean btn-icon btn-icon-md delete-button" title="{{__('dashboard.delete')}}" data-toggle="modal" data-target="#delete_modal" data-url="{{ route('dashboard.applications.destroy',$application->id) }}" data-item-id="{{ $application->id }}">
                                    <i class="flaticon2-trash trash-icon"></i>
                                </a>
                                @endif
                            </td>
                ...
                {!! str_replace('/?', '?', $applications->withQueryString()->render()) !!}
            ...
            <div class="card-body">
                ...
                <div class="card-toolbar" style=" text-align: center; background-color: #f2f3f8 ;padding: 100px;  color: #101010">
                    ...
                    <a href="{{route('dashboard.applications.create')}}" class="btn btn-primary mt-2">
                        <i class="la la-plus"></i>@lang('dashboard.new_application')</a>
                </div>
            </div>
            @endif
        </div>
