...
<ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
    ...
    <li class="breadcrumb-item text-muted">
        <a href="{{url('stores')}}" class="text-muted">@lang('dashboard.stores')</a>
    </li>
    <li class="breadcrumb-item text-muted">
...
<script>
    ...
    $(document).ready(function() {
        var conType = $("#con_type option:selected").val();
        var addressType = $('select[name="address_type"]').val();
        
        if (conType == 'online') {
            $('#place_type').css('display', 'none');
            $('#link_address').css('display', 'block');
            $('.map').css('display', 'none');
        } else if (conType == 'local') {
            $('#place_type').css('display', 'block');
            // Use the saved address_type value from database, or default to 'link' if not set
            if (!addressType || addressType == '') {
                $('select[name="address_type"]').val('link').trigger('change');
                if ($('select[name="address_type"]').hasClass('select2-hidden-accessible')) {
                    $('select[name="address_type"]').trigger('change.select2');
                }
                $('#link_address').css('display', 'block');
                $('.map').css('display', 'none');
            } else if (addressType == 'map') {
                // Keep the saved 'map' value
                $('#link_address').css('display', 'none');
                $('.map').css('display', 'block');
            } else if (addressType == 'link') {
                // Keep the saved 'link' value
                $('#link_address').css('display', 'block');
                $('.map').css('display', 'none');
            }
        }
    ...
    $("#con_type").change(
        function() {
            ...
            if (getSort == 'online') {
                ...
                $('#link_address').css('display', 'block');
                $('.map').css('display', 'none');
                // Keep region and city fields visible but make them optional
                $('#region_field').css('display', 'block');
                $('#city_field').css('display', 'block');
                // Remove required attribute
                $('#region_id').removeAttr('required');
                $('#city_id').removeAttr('required');
                // Hide required asterisk
                $('.region-required').css('display', 'none');
                $('.city-required').css('display', 'none');

            } else if (getSort == 'local') {
                $('#place_type').css('display', 'block');
                // Always set default to 'link' when local is selected
                $('select[name="address_type"]').val('link').trigger('change');
                ...
                $('#link_address').css('display', 'block');
                $('.map').css('display', 'none');
                // Keep region and city fields visible and make them required
                $('#region_field').css('display', 'block');
                $('#city_field').css('display', 'block');
                ...
                $('.region-required').css('display', 'inline');
                $('.city-required').css('display', 'inline');
    ...
    $('select[name="address_type"]').change(
        function() {
            var getSort = $(this).val();
            var conType = $("#con_type option:selected").val();

            // Don't show map if con_type is 'online'
            if (conType == 'online') {
                $('.map').css('display', 'none');
                return;
            }
    ...
    $(document).on('change', '#region_id', function (){
        ...
        $.ajax({
            url:'/setting/get-cities?region_id='+regionId,
            method:'get',
    ...
    // Initialize on page load
    $(document).ready(function() {
        var initialConType = $("#con_type option:selected").val();
        if (initialConType == 'online') {
            $('#region_id').removeAttr('required');
            $('#city_id').removeAttr('required');
            $('.region-required').css('display', 'none');
            $('.city-required').css('display', 'none');
                            ...
                            <div class="card card-custom gutter-b">
                                ...
                                <div class="card-body custom-nav">
                                    ...
                                    <div class="tab-content" id="myTabContent">
                                        <div class="tab-pane fade show active" id="info" role="tabpanel" aria-labelledby="info-tab">@include('dashboard.stores.includes._global_edit')</div>
                                        ...
                                        <div class="tab-pane fade show" id="related" role="tabpanel" aria-labelledby="location-tab">@include('dashboard.stores.includes._global_related')</div>
                                        <div class="tab-pane fade show" id="near" role="tabpanel" aria-labelledby="location-tab">@include('dashboard.stores.includes._global_near')</div>
                                    </div>
